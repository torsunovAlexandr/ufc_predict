...
## Если котировки не сохраняются

Первым делом — пошаговая диагностика, она проходит всю цепочку от ключа
до сопоставления имён и говорит, где рвётся:

```bash
php artisan ufc:odds-doctor          # или make odds-doctor
php artisan ufc:odds-doctor --live   # с реальным запросом, минуя кэш ответа
```

Пять причин, по которым «сохранено 0», и как их различить:

| Код `reason` | Что произошло |
|---|---|
| `no_fights` | у турнира нет боёв в базе — сначала `make events` |
| `no_provider` | не задан `ODDS_API_KEY` либо исчерпана дневная квота |
| `empty_response` | источник ответил, но котировок не прислал |
| `no_match` | котировки пришли, но не совпали с боями по именам |
| `null` | всё хорошо, что-то сохранено |

Самый частый случай — `no_match`. The Odds API отдаёт ближайшие бои по **всему**
ММА, включая другие промоушены, поэтому в ответе легко может не оказаться боёв
именно вашего турнира. `ufc:odds-doctor` печатает даты боёв из ответа рядом
с парами имён — если даты не совпадают с датой турнира, дело именно в этом.

Вторая по частоте причина — расхождение в написании имён. Сопоставление идёт
по полному имени и по фамилии (`OddsService::matchFight`); если букмекер пишет
`Jon Jones`, а в базе после парсинга оказалось `Jonathan Jones`, совпадения
не будет. Поправьте имя бойца в базе или расширьте сопоставление.

Отдельная мина: **кэш конфигурации**. Если когда-то выполнялся `config:cache`,
новый `ODDS_API_KEY` из `.env` не подхватится. `ufc:odds-doctor` это проверяет;
лечится командой `make cache-clear`.
